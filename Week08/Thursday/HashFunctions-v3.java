...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < TABLE_SIZE; i++) {
            frequency.put(i, 0);
            entries.put(i, new ArrayList<String>());
        }

        int collisions = 0;

        for(int i = 0; i < pokemonNames.length; i++) {
            int index = hash(pokemonNames[i],3);
            ...
            entries.get(index).add(pokemonNames[i]);
        }

        for (int i = 0; i < TABLE_SIZE; i++) {
            System.out.println("Index " + i + " had " + frequency.get(i) + " occurrences.");
            System.out.println("Contents: " + entries.get(i));
        }
    ...
    static int hash(Object o, int option) {
        switch(option) {
            ...
            case 3:
                // multiplication
                final double MULTIPLICATION_FACTOR = 0.6180339887;
